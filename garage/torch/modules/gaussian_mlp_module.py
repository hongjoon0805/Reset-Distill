...
class GaussianMLPBaseModule(nn.Module):
    ...
    Args:
        ...
        n_tasks (int): The number of total tasks
    ...
    def __init__(self,
        ...
        if isinstance(output_dim, list):
            log_stds = [torch.Tensor([init_std] * output_dim_per_task).log() for output_dim_per_task in output_dim]
            if self._learn_std:
                self._log_std = nn.ParameterList([nn.Parameter(log_std) for log_std in log_stds])
            else:
                self._log_std = log_stds
    ...
    def forward(self, *inputs, seq_idx, features=None):
        """Forward method.
        ...
        """
        outputs = self._get_mean_and_log_std(*inputs, features=features)
        if isinstance(seq_idx, int):
            if isinstance(outputs, tuple):
                if isinstance(outputs[1], nn.ParameterList):
                    mean, log_std_uncentered = outputs[0][seq_idx], outputs[1][seq_idx]
                else:
                    mean, log_std_uncentered = outputs[0][seq_idx], outputs[1]
            else:
                mean, log_std_uncentered = outputs[2*seq_idx], outputs[2*seq_idx+1]
        else:
            if isinstance(outputs, tuple):
                means, log_std_uncentered = outputs[0], outputs[1]
                means = torch.stack(means, dim=-1)
                idx_list = torch.arange(len(means))
                mean = means[idx_list, :, seq_idx]
            else:
                outputs = torch.stack(outputs, dim=-1)
                idx_list = torch.arange(len(outputs))
                mean, log_std_uncentered = outputs[idx_list, :, 2*seq_idx], outputs[idx_list, :, 2*seq_idx+1]
...
class GaussianMLPModule(GaussianMLPBaseModule):
    ...
    def _get_mean_and_log_std(self, x, features=None):
        ...
        """
        mean = self._mean_module(x, features=features)
        self._feature = self._mean_module._feature
        self._features = self._mean_module._features
        self._stats = self._mean_module._stats

        return mean, self._log_std
    def reset_parameter(self, column=True, adaptor=False):
        self._mean_module.reset_parameter(column=column, adaptor=adaptor)
...
class GaussianMLPTwoHeadedModule(GaussianMLPBaseModule):
    ...
    def _get_mean_and_log_std(self, *inputs, features=None):
        """Get mean and std of Gaussian distribution given inputs.
        ...
        ret = self._shared_mean_log_std_network(*inputs, features=features)
        self._feature = self._shared_mean_log_std_network._feature
        self._features = self._shared_mean_log_std_network._features
        self._stats = self._shared_mean_log_std_network._stats
        return ret
    def reset_parameter(self, column=True, adaptor=False):
        self._shared_mean_log_std_network.reset_parameter(column=column, adaptor=adaptor)
