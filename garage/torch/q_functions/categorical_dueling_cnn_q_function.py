...
class CategoricalDuelingCNNQFunction(nn.Module):
    """Categorical Dueling CNN Q Function.
    ...
    def __init__(self,
        ...
        self._val = MLPModule(flat_dim,
                              ...
                              layer_normalization=layer_normalization, 
                              noisy_net=noisy_net)
        self._adv = MLPModule(flat_dim,
                              env_spec.action_space.flat_dim*atom_size,
                              hidden_sizes,
                              n_tasks=n_tasks,
                              hidden_nonlinearity=mlp_hidden_nonlinearity,
                              hidden_w_init=hidden_w_init,
        ...
        self._support = torch.linspace(v_min, v_max, atom_size).cuda()
    ...
    def forward(self, observations, seq_idx):
        ...
        dist = self.dist(observations, seq_idx)
        q = torch.sum(dist * self._support, dim=2)

        return q
        
    
    def dist(self, observations, seq_idx):

        # We're given flattened observations.
        observations = observations.reshape(
            -1, *self._env_spec.observation_space.shape)
        out = self._module(observations)
        val = self._val(out)[seq_idx].view(-1, 1, self._atom_size)
        adv = self._adv(out)[seq_idx].view(-1, self._env_spec.action_space.flat_dim, self._atom_size)
    ...
    def reset_noise(self):
        ...
        self._val.reset_noise()
        self._adv.reset_noise()